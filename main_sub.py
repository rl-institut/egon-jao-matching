...
def main():
    ...
    try:
        ...
        if 'name' in dlr_buses_df.columns:
            ...
            logger.error("'name' column not found in dlr_buses_df. Cannot rename to 'id_dlr'.")
            raise KeyError("'name' column is missing. Ensure dlr_buses data has a 'name' column.")
        ...
        filtered_network_buses_df = filter_network_substations(network_buses_df, min_voltage_level)
        ...
        substations_gdf, dlr_buses_gdf = prepare_geodataframes(filtered_network_buses_df, filtered_dlr_buses_df, config)

        # Filter inside Germany and exclude 110 kV or lower
        substations_gdf = filter_network_substations_inside_germany(substations_gdf, germany_gdf)

        # Filter DLR inside Germany
        dlr_buses_gdf = filter_dlr_substations_inside_germany(dlr_buses_gdf, germany_gdf)

        # Perform spatial matching
        matched_substations_gdf, unmatched_substations_gdf, matched_dlr_buses_gdf, unmatched_dlr_buses_gdf = spatial_match_substations(
            substations_gdf, dlr_buses_gdf, config
        )

        # Save matched and unmatched substations to CSV
        save_csv_files(matched_substations_gdf, unmatched_substations_gdf, unmatched_dlr_buses_gdf, config)

        # Combine matched and unmatched network substations
        total_network_substations_gdf = pd.concat([matched_substations_gdf, unmatched_substations_gdf], ignore_index=True)

        # Check if we have any substations at all
        if total_network_substations_gdf.empty:
            logger.warning("No network substations remain after matching. Skipping DLR counting and mapping.")
            logger.info("Substation matching process completed with no substations to display.")
            return
        ...
        total_network_substations_gdf['bus_id'] = total_network_substations_gdf['bus_id'].astype(str)
        ...
        total_network_substations_gdf = total_network_substations_gdf.merge(
            counts_df_with_has_dlr,
            on='bus_id',
            how='left'
        )

        # After merging
        total_network_substations_gdf['num_dlr_substations'] = total_network_substations_gdf[
            'num_dlr_substations'].fillna(0).astype(int)
        total_network_substations_gdf['has_dlr_without_T'] = total_network_substations_gdf['has_dlr_without_T'].fillna(
            False)

        # Filter substations with more than one DLR substation
        filtered_substations_gdf = total_network_substations_gdf[total_network_substations_gdf['num_dlr_substations'] > 1].copy()
        ...
        filtered_substations_gdf = gpd.GeoDataFrame(filtered_substations_gdf, geometry='geometry', crs=projected_crs).to_crs(original_crs)
        ...
        logger.debug(f"After filter_buses_by_transformers: {len(filtered_network_buses_df)}")
        ...
        total_network_substations_gdf = gpd.GeoDataFrame(total_network_substations_gdf, geometry='geometry', crs=projected_crs).to_crs(original_crs)
        total_network_substations_gdf = total_network_substations_gdf[total_network_substations_gdf['v_nom'] >= 110]
