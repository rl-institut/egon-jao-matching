...
def main():
    ...
    network_transformers["bus0"] = (
        network_transformers["bus0"].astype(str).str.strip().str.upper()
    )
    network_transformers["bus1"] = (
        network_transformers["bus1"].astype(str).str.strip().str.upper()
    )

    dlr_transformers["bus0"] = (
        dlr_transformers["bus0"].astype(str).str.strip().str.upper()
    )
    dlr_transformers["bus1"] = (
        dlr_transformers["bus1"].astype(str).str.strip().str.upper()
    )

    matched_substations["bus_id"] = (
        matched_substations["bus_id"].astype(str).str.strip().str.upper()
    )
    matched_substations["id_dlr"] = (
        matched_substations["id_dlr"].astype(str).str.strip().str.upper()
    )

    # Identify unmatched id_dlr before merging
    unmatched_id_dlr = matched_substations[
        ~matched_substations["id_dlr"].isin(dlr_transformers["bus1"])
    ]
    logger.info(
        f"Number of unmatched 'id_dlr' values before merging: {unmatched_id_dlr.shape[0]}"
    )
    if not unmatched_id_dlr.empty:
        ...
        unmatched_id_dlr.to_csv(
            os.path.join(data_dir, "unmatched_id_dlr_before_merge.csv"), index=False
        )
        logger.info(
            f"Unmatched 'id_dlr' entries saved to 'unmatched_id_dlr_before_merge.csv'."
        )
    ...
    logger.info(
        f"Number of network transformers after inner merge: {merged_df.shape[0]}"
    )  # Expected: 83
    ...
    logger.info(
        f"Number of matched network transformers after merging with DLR transformers: {final_merged_df.shape[0]}"
    )  # Expected: 83
    ...
    dlr_attributes = [
        ...
        "Voltage_level(kV) Primary",
        "Voltage_level(kV) Secondary",
        "Taps used for RAO",
        "Theta θ (°)",
    ...
    missing_attributes = [
        attr
        for attr in dlr_attributes + dlr_overlapping_attributes
        if attr not in dlr_transformers.columns
    ]
    if missing_attributes:
        logger.error(
            f"The following DLR attributes are missing in 'dlr_transformers.csv': {missing_attributes}"
        )
        sys.exit(1)
    ...
    logger.info(
        f"Enriched matched network transformers with DLR attributes saved to '{output_file}'."
    )
    ...
    logger.info(
        f"Total matched network transformers with DLR attributes: {total_matched_transformers}"
    )
    logger.info(
        f"Number of matched substations without DLR transformer attributes: {unmatched_transformers}"
    )
    ...
    if unmatched_transformers > 0:
        ...
        unmatched_entries.to_csv(
            os.path.join(data_dir, "unmatched_matched_substations.csv"), index=False
        )
        logger.info(
            f"Unmatched matched substations saved to 'unmatched_matched_substations.csv'."
        )
