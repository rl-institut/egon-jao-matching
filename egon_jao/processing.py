...
def prepare_geodataframes(dlr_buses, dlr_lines, network_buses, network_lines):
    ...
    dlr_lines["geometry"] = dlr_lines.apply(
        lambda row: LineString([row["bus0_geom"], row["bus1_geom"]]), axis=1
    )
    ...
    if unique_count == total_count:
        ...
        network_buses_gdf["bus_idx"] = (
            network_buses_gdf["scn_name"].astype(str)
            + "_"
            + network_buses_gdf["unique_idx"].astype(str)
        )
    ...
    if "geom" in network_lines_filtered.columns:
        network_lines_filtered["geometry"] = gpd.GeoSeries.from_wkt(
            network_lines_filtered["geom"]
        )
...
def match_buses_with_nearest(network_buses_gdf, dlr_buses_gdf, max_distance=5000):
    ...
    # Get DLR bus names
    matched_buses["dlr_bus_name"] = dlr_buses_proj.loc[
        matched_buses["index_right"], "name"
    ].values
    ...
    bus_id_mapping = bus_id_mapping.sort_values("distance").drop_duplicates(
        subset=["bus_idx"], keep="first"
    )
...
def match_lines_based_on_matched_buses(network_lines_matched, dlr_lines_gdf):
    ...
    print(
        f"Number of matched network lines (Nearest Neighbor): {len(matched_network_lines)}"
    )
...
def match_lines_with_buffer(
    unmatched_network_lines_gdf,
    dlr_lines_gdf,
    buffer_distance=10000,
    matched_network_line_ids=None,
    matched_dlr_line_ids=None,
):
    """
    ...
    try:
        ...
        for idx, dlr_row in dlr_lines_proj.iterrows():
            dlr_line_id = dlr_row.get(
                "id", idx
            )  # Ensure we have a unique identifier for DLR lines
            ...
            if not network_matches.empty:
                ...
                network_matches["intersection_area"] = (
                    network_matches.geometry.intersection(dlr_buffer).area
                )
                best_match = network_matches.loc[
                    network_matches["intersection_area"].idxmax()
                ]
        ...
        if additional_matched_network_lines:
            ...
            additional_matched_network_lines_gdf = (
                additional_matched_network_lines_gdf.to_crs("EPSG:4326").reset_index(
                    drop=True
                )
            )
        else:
            additional_matched_network_lines_gdf = gpd.GeoDataFrame(
                columns=unmatched_network_lines_gdf.columns
            )
        ...
        if additional_matched_dlr_lines:
            ...
            additional_matched_dlr_lines_gdf = additional_matched_dlr_lines_gdf.to_crs(
                "EPSG:4326"
            ).reset_index(drop=True)
        else:
            additional_matched_dlr_lines_gdf = gpd.GeoDataFrame(
                columns=dlr_lines_gdf.columns
            )
...
def filter_dlr_lines_inside_germany(dlr_lines_gdf, germany_gdf):
    ...
    dlr_lines_inside_germany = gpd.overlay(
...
def create_folium_map(
    ...
    network_buses_gdf,
    dlr_buses_gdf,
    matched_network_lines_nn=None,
    matched_dlr_lines_nn=None,
    matched_network_lines_buffer=None,
    matched_dlr_lines_buffer=None,
):
    """
    ...
    with contextlib.suppress(Exception):
        ...
        if matched_network_lines_nn is not None and not matched_network_lines_nn.empty:
            folium.GeoJson(
                ...
                tooltip=folium.GeoJsonTooltip(
                    fields=["name"], aliases=["Matched Line NN"], localize=True
                ),
            ).add_to(folium_map)
        else:
        ...
        if matched_dlr_lines_nn is not None and not matched_dlr_lines_nn.empty:
            folium.GeoJson(
                ...
                tooltip=folium.GeoJsonTooltip(
                    fields=["name"], aliases=["Matched DLR Line NN"], localize=True
                ),
            ).add_to(folium_map)
        else:
        ...
        if (
            matched_network_lines_buffer is not None
            and not matched_network_lines_buffer.empty
        ):
            folium.GeoJson(
                ...
                tooltip=folium.GeoJsonTooltip(
                    fields=["name"], aliases=["Matched Line Buffer"], localize=True
                ),
            ).add_to(folium_map)
        else:
        ...
        if matched_dlr_lines_buffer is not None and not matched_dlr_lines_buffer.empty:
            folium.GeoJson(
                ...
                tooltip=folium.GeoJsonTooltip(
                    fields=["name"], aliases=["Matched DLR Line Buffer"], localize=True
                ),
            ).add_to(folium_map)
        else:
