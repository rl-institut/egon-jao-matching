...
def filter_network_substations(network_buses_df, min_voltage_level):
    ...
    substations_df = network_buses_df[
        network_buses_df["v_nom"] >= min_voltage_level
    ].copy()
    logger.info(
        f"Filtered network buses to {len(substations_df)} substations with voltage >= {min_voltage_level} kV."
    )
...
def prepare_geodataframes(substations_df, dlr_buses_df, config):
    ...
    if "bus_id" not in substations_gdf.columns:
        substations_gdf = substations_gdf.reset_index().rename(
            columns={"index": "bus_id"}
        )
...
def filter_dlr_substations_inside_germany(dlr_buses_gdf, germany_gdf):
    ...
    dlr_buses_in_germany = gpd.sjoin(
        dlr_buses_gdf, germany_gdf, how="inner", predicate="intersects"
    )
    dlr_buses_in_germany = dlr_buses_in_germany.drop(
        columns=["index_right"]
...
def filter_network_substations_inside_germany(substations_gdf, germany_gdf):
    ...
    logger.debug(
        f"Number of substations before filtering inside Germany: {len(substations_gdf)}"
    )

    # Ensure CRS matches
    if substations_gdf.crs != germany_gdf.crs:
        logger.debug(
            f"Reprojecting substations from {substations_gdf.crs} to {germany_gdf.crs}"
        )
        substations_gdf = substations_gdf.to_crs(germany_gdf.crs)

    # Spatial join to determine which substations are inside Germany
    substations_in_germany = gpd.sjoin(
        substations_gdf, germany_gdf, how="inner", predicate="intersects"
    )
    substations_in_germany = substations_in_germany.drop(
        columns=["index_right"]
    ).reset_index(drop=True)

    logger.debug(
        f"Number of substations after spatial join: {len(substations_in_germany)}"
    )
    ...
    logger.debug(
        f"Number of substations after excluding <=110 kV: {len(substations_filtered)} (from {initial_count})"
    )

    logger.info(
        f"Network substations inside Germany after excluding <=110 kV: {len(substations_filtered)}"
    )
...
def filter_buses_by_transformers(
    network_buses_df, dlr_buses_df, network_transformers_file, dlr_transformers_file
):
    """
    Filters the network and DLR buses to only those connected to transformers.
    """
    transformer_buses_network = get_transformer_buses_from_network_transformers(
        network_transformers_file
    )
    transformer_buses_dlr = get_transformer_buses_from_dlr_transformers(
        dlr_transformers_file
    )

    # Filter network buses using 'bus_id'
    network_buses_filtered = network_buses_df[
        network_buses_df["bus_id"].isin(transformer_buses_network)
    ].copy()

    # Filter DLR buses
    if "id_dlr" in dlr_buses_df.columns:
        dlr_buses_filtered = dlr_buses_df[
            dlr_buses_df["id_dlr"].isin(transformer_buses_dlr)
        ].copy()
    else:
    ...
    logger.debug(
        f"After filter_buses_by_transformers: {len(network_buses_filtered)} network buses, {len(dlr_buses_filtered)} DLR buses."
    )
...
def spatial_match_substations(substations_gdf, dlr_buses_gdf, config):
    ...
    if not substations_gdf["bus_id"].is_unique:
        logger.error(
            "Duplicate 'bus_id' values found in network substations. Please ensure 'bus_id' is unique."
        )
    ...
    if not dlr_buses_gdf["id_dlr"].is_unique:
        logger.error(
            "Duplicate 'id_dlr' values found in DLR substations. Please ensure 'id_dlr' is unique."
        )
    ...
    substations_buffer_gdf = substations_gdf[["bus_id", "buffer"]].set_geometry(
        "buffer"
    )

    # Perform spatial join to find DLR substations within buffer zones
    possible_matches = gpd.sjoin(
        substations_buffer_gdf, dlr_buses_gdf, how="left", predicate="intersects"
    )
    ...
    network_geom = substations_gdf.set_index("bus_id").loc[
        possible_matches["bus_id"], "geometry"
    ]
    ...
    matched_substations_gdf = (
        substations_gdf.set_index("bus_id").loc[list(matched_network_ids)].copy()
    )
    matched_substations_gdf = matched_substations_gdf.reset_index()
    matched_substations_gdf["id_dlr"] = dlr_buses_gdf.loc[
        list(matched_dlr_ids), "id_dlr"
    ].values
    ...
    unmatched_substations_gdf = substations_gdf[
        substations_gdf["bus_id"].isin(unmatched_network_ids)
    ].copy()
    ...
    logger.info(
        f"Total unmatched network substations: {len(unmatched_substations_gdf)}"
    )
...
def save_csv_files(
    matched_substations_gdf, unmatched_substations_gdf, unmatched_dlr_buses_gdf, config
):
    """Saves the matched and unmatched substations to CSV files."""
    # Reproject back to original CRS if needed
    matched_substations_gdf = matched_substations_gdf.to_crs(
        config["coordinate_reference_systems"]["crs"]
    )
    unmatched_substations_gdf = unmatched_substations_gdf.to_crs(
        config["coordinate_reference_systems"]["crs"]
    )
    unmatched_dlr_buses_gdf = unmatched_dlr_buses_gdf.to_crs(
        config["coordinate_reference_systems"]["crs"]
    )

    # **Save matched substations**
    matched_substations_df = matched_substations_gdf.drop(
        columns=["geometry", "index_right"], errors="ignore"
    )
    matched_substations_df.to_csv(
        config["output_files"]["matched_substations_csv"], index=False
    )
    logger.info(
        f"Matched substations saved to '{config['output_files']['matched_substations_csv']}'."
    )

    # **Save unmatched substations**
    unmatched_substations_df = unmatched_substations_gdf.drop(
        columns=["geometry"], errors="ignore"
    )
    unmatched_substations_df.to_csv(
        config["output_files"]["unmatched_substations_csv"], index=False
    )
    logger.info(
        f"Unmatched substations saved to '{config['output_files']['unmatched_substations_csv']}'."
    )

    # **Save unmatched DLR substations**
    unmatched_dlr_buses_df = unmatched_dlr_buses_gdf.drop(
        columns=["geometry"], errors="ignore"
    )
    unmatched_dlr_buses_df.to_csv(
        config["output_files"]["unmatched_dlr_substations_csv"], index=False
    )
    logger.info(
        f"Unmatched DLR substations saved to '{config['output_files']['unmatched_dlr_substations_csv']}'."
    )


def create_matched_unmatched_maps(
    matched_substations_gdf,
    unmatched_substations_gdf,
    matched_dlr_buses_gdf,
    unmatched_dlr_buses_gdf,
    ...
    create_folium_map_with_circles(
        ...
        gpd.GeoDataFrame(
            columns=["id_dlr", "geometry"], crs="EPSG:4326"
        ),  # Empty GeoDataFrame with correct columns and CRS
...
def create_folium_map_with_circles(
    substations_gdf,
    matched_dlr_buses_gdf,
    unmatched_dlr_buses_gdf,
    output_path,
    ...
    unmatched_dlr_buses_gdf = (
        unmatched_dlr_buses_gdf.to_crs("EPSG:4326")
    ...
    if not all(geom_type == "Point" for geom_type in geom_types):
        logger.error(
            f"Not all geometries are Points in '{map_title}'. Skipping mapping."
        )
        return
...
def create_total_map(
    ...
    try:
        ...
        network_substations_gdf = network_substations_gdf[
            network_substations_gdf["v_nom"] > 110
        ]
        logger.info(
            f"Number of network substations after excluding 110 kV: {len(network_substations_gdf)}"
        )
        ...
        if not all(geom_type == "Point" for geom_type in geom_types):
            logger.error(
                "Not all geometries are Points in 'Total Map'. Skipping mapping."
            )
            return
        else:
        ...
        for idx, row in network_substations_gdf.iterrows():
            try:
                ...
                logger.debug(
                    f"Processing Network Substation ID: {row['bus_id']} with num_dlr_substations: {num_dlr}"
                )
        ...
        logger.info(
            f"Network substations added to the map. Number of orange markers: {orange_count}"
        )
        ...
        if not matched_dlr_buses_gdf.empty:
            ...
            for idx, row in matched_dlr_buses_gdf.iterrows():
                try:
                    logger.debug(
                        f"Adding Matched DLR Substation ID: {row['id_dlr']} at (Lat: {row.geometry.y}, Lon: {row.geometry.x})"
                    )
                    folium.CircleMarker(
                        location=(row.geometry.y, row.geometry.x),
                        radius=7,
                    ...
                    logger.error(
                        f"Error adding matched DLR substation ID {row['id_dlr']}: {e}"
                    )
        else:
        ...
        if not unmatched_dlr_buses_gdf.empty:
            logger.info(
                f"Adding {len(unmatched_dlr_buses_gdf)} unmatched DLR substations."
            )
            for idx, row in unmatched_dlr_buses_gdf.iterrows():
                try:
                    logger.debug(
                        f"Adding Unmatched DLR Substation ID: {row['id_dlr']} at (Lat: {row.geometry.y}, Lon: {row.geometry.x})"
                    )
                    folium.CircleMarker(
                        location=(row.geometry.y, row.geometry.x),
                        radius=7,
                    ...
                    logger.error(
                        f"Error adding unmatched DLR substation ID {row['id_dlr']}: {e}"
                    )
        else:
...
def create_total_map_test(
    ...
    if not all(geom_type == "Point" for geom_type in geom_types):
        logger.error(
            "Not all geometries are Points in 'Test Total Map'. Skipping mapping."
        )
        return
    else:
        logger.info("All geometries in network_substations_gdf are Points.")
    ...
    for idx, row in network_substations_gdf.iterrows():
        try:
            logger.debug(
                f"Processing Network Substation ID: {row['bus_id']} at (Lat: {row.geometry.y}, Lon: {row.geometry.x})"
            )
    ...
    if not matched_dlr_buses_gdf.empty:
        logger.info(
            f"Adding {len(matched_dlr_buses_gdf)} matched DLR substations to the test map."
        )
        for idx, row in matched_dlr_buses_gdf.iterrows():
            try:
                logger.debug(
    ...
    if not unmatched_dlr_buses_gdf.empty:
        logger.info(
            f"Adding {len(unmatched_dlr_buses_gdf)} unmatched DLR substations to the test map."
        )
        for idx, row in unmatched_dlr_buses_gdf.iterrows():
            try:
                logger.debug(
    ...
    try:
        ...
        logger.info(
            "Test Total Map saved to 'results/maps/test_total_substations_map.html'."
        )
    except Exception as e:
...
def count_dlr_substations_in_buffers(network_substations_gdf, dlr_buses_gdf, config):
    ...
    logger.info(
        "Counting DLR substations within each network substation's buffer zone."
    )
    ...
    if dlr_buses_gdf.crs != network_substations_gdf.crs:
        logger.info(
            f"Reprojecting DLR buses from {dlr_buses_gdf.crs} to {network_substations_gdf.crs}."
        )
    ...
    grouped = (
        unique_assignments.groupby("bus_id")
        .agg({"id_dlr": lambda x: list(x)})
        .reset_index()
    )
    ...
    counts_full = network_substations_gdf[["bus_id"]].merge(
        grouped[["bus_id", "num_dlr_substations", "id_dlr"]], on="bus_id", how="left"
    )

    # Fill NaN values in 'num_dlr_substations' with 0
    counts_full["num_dlr_substations"] = (
        counts_full["num_dlr_substations"].fillna(0).astype(int)
    )

    # Fill NaN values in 'id_dlr' with empty lists
    counts_full["id_dlr"] = counts_full["id_dlr"].apply(
...
def save_dlr_substations_count_csv(counts_df, config):
    ...
    try:
        ...
        for i in range(max_dlr_ids):
            counts_df[f"dlr_id_{i + 1}"] = counts_df["id_dlr"].apply(
        ...
        logger.info(
            f"DLR substations count per network substation saved to '{output_path}'."
        )
        ...
        logger.error(
            f"An unexpected error occurred while saving DLR substations count CSV: {e}"
        )
        raise


def create_filtered_substations_map(
    network_substations_gdf, dlr_buses_gdf, config, germany_gdf
):
    """
    ...
    if not all(geom_type == "Point" for geom_type in geom_types):
        logger.error(
            "Not all geometries are Points in network_substations_gdf. Skipping mapping."
        )
        return
    ...
    logger.debug(
        f"Columns in network_substations_proj after reprojection: {network_substations_proj.columns.tolist()}"
    )

    # Create buffers around each network substation
    network_substations_proj["buffer"] = network_substations_proj.geometry.buffer(
        buffer_radius_meters
    )
    logger.info(
        f"Buffers of {buffer_radius_meters} meters created around network substations."
    )

    # Create a GeoDataFrame for buffers
    buffers_gdf = gpd.GeoDataFrame(
        network_substations_proj[
            ["bus_id", "num_dlr_substations", "has_dlr_without_T"]
        ],
    ...
    for idx, row in network_substations_gdf.iterrows():
        try:
            ...
            buffer_geom = buffers_gdf.loc[
                buffers_gdf["bus_id"] == bus_id, "geometry"
            ].values[0]
            ...
            logger.debug(
                f"Found {len(dlr_in_buffer)} DLR substations within buffer of Substation ID: {bus_id}"
            )
...
def clean_identifiers(df: pd.DataFrame, bus_col: str, prefix: str = "") -> pd.DataFrame:
    ...
    if prefix:
        df[bus_col] = df[bus_col].apply(
            lambda x: x if x.startswith(prefix) else f"{prefix}{x}"
        )
    return df
