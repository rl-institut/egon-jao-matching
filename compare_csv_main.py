...
def extract_b_values(df, component):
    ...
    if b_col and b_col in df.columns:
        ...
        print(f"Extracted 'b' values for {component}. Count after cleaning: {b_series.count()}")
        return b_series
    else:
        available_cols = df.columns.tolist()
        print(f"Column '{b_col}' not found in the dataset for '{component}'. Available columns: {available_cols}")
        raise ValueError(f"Column '{b_col}' not found in the dataset for '{component}'.")
...
def perform_hypothesis_test(original, updated):
    ...
    if normal:
        ...
        test_stat, p_value = stats.mannwhitneyu(original, updated, alternative='two-sided')
...
def plot_histogram(original, updated, component, b_label):
    ...
    plt.figure(figsize=(8, 6))
    sns.histplot(original, color='blue', label='Original', kde=True, stat="density", linewidth=0, alpha=0.6)
    sns.histplot(updated, color='orange', label='Updated', kde=True, stat="density", linewidth=0, alpha=0.6)
    plt.xlabel(b_label)
    plt.ylabel('Density')
...
def identify_extreme_changes_b(original_df, updated_df, component):
    ...
    extreme_changes = merged_df[(merged_df['b_ratio'] > 10) | (merged_df['b_ratio'] < 0.1)]
...
def plot_extreme_changes_b(extreme_df, component, b_label):
    ...
    sns.scatterplot(data=extreme_df, x='b_original', y='b_updated', hue='b_ratio', palette='coolwarm', size='b_ratio',
...
def plot_corresponding_b(original_df, updated_df, component, b_label):
    ...
    sns.scatterplot(data=merged_df, x='b_original', y='b_updated', alpha=0.3, edgecolor=None)
    plt.plot([merged_df['b_original'].min(), merged_df['b_original'].max()],
...
def main():
    ...
    for component in ['transformers', 'lines']:
        ...
        if original_key in b_values and updated_key in b_values:
            result = perform_hypothesis_test(b_values[original_key], b_values[updated_key])
            result['Component'] = component
            hypothesis_results.append(result)
            print(f"Performed {result['Test']} for {component}. p-value = {result['p-value']:.4f}")
        else:
    ...
    if hypothesis_results:
        ...
        print(hypothesis_df[['Component', 'Test', 'Test Statistic', 'p-value', 'Significant (α=0.05)']])

        # Save to CSV
        hypothesis_output_path = os.path.join(OUTPUT_DIR, 'hypothesis_testing_results.csv')
    ...
    for component in ['transformers', 'lines']:
        ...
        if original_key in b_values and updated_key in b_values:
            ...
            plot_boxplot(b_values[original_key], b_values[updated_key], component, b_label)
            plot_histogram(b_values[original_key], b_values[updated_key], component, b_label)
            plot_corresponding_b(datasets[original_key], datasets[updated_key], component, b_label)
        else:
    ...
    for component in ['transformers', 'lines']:
        ...
        if original_key in b_values and updated_key in b_values:
            ...
            identifier = IDENTIFIER_COLUMNS.get(component)
            if isinstance(identifier, list):
                missing_cols = [col for col in identifier if
                                col not in original_df.columns or col not in updated_df.columns]
                if missing_cols:
                    print(
                        f"Identifier columns {missing_cols} not found in one of the datasets for '{component}'. Skipping extreme change identification.")
                    continue
            else:
                if identifier not in original_df.columns or identifier not in updated_df.columns:
                    print(
                        f"Identifier column '{identifier}' not found in one of the datasets for '{component}'. Skipping extreme change identification.")
                    continue

            try:
                extreme_changes = identify_extreme_changes_b(original_df, updated_df, component)

                if not extreme_changes.empty:
                    plot_extreme_changes_b(extreme_changes, component, B_COLUMNS.get(component, 'b'))
                else:
